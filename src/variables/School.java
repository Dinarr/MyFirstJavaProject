package variables;

public class School {	
// project
// package under the src folder
// java class inside the package 
// comment we can write for ourselves
// something 

	public static void main(String[] args) {
	    System.out.println("Class started");
	    System.out.println("Session in process"); 
	    int age, size, numberOfStudents;
	    size = 44;
	    age = 24;
	    numberOfStudents = 55;
	    System.out.println(age);
	    System.out.println(size);
	    System.out.println(numberOfStudents);
	    		
	       
	    // camel case
	    // start with lower case and Upper case for
	    //each new word
	    int numOfComputersInCybertek = 4; 
	    System.out.println(numOfComputersInCybertek);
	    byte width = 39;
	    short population = 20000;
	    double price = 42.99;
	    float price2 = 32.45f;
	    
	    char grade; // character. You need to use single quotation
	                // for a value. It can store only single character. 
	    grade = 'A';
	    System.out.println(grade);
//    'A' vs "A" --> '' for char and it's a rule. "" -> for String
//    String is a sequence of character.
//    23
//    int, byte, short,long, double,
//    #, @, &
	  char specialChar = '#';
//    23 -> '23'. No. 23 is two digit/ two characters 
     // boolean
	  boolean snowing;
	  snowing = true; 
	  boolean oktaVerifyResponse = true; 
	    
	}
	
	
}
